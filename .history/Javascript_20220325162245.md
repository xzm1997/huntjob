...
- **原理**：图片进入可视区域之后再去请求图片资源。
  ...
  - 在图片进入可视化区域之前，`img`标签上的`src`属性并没有被设置。在`img`标签上有一个`data-url`属性。该属性的值存放着图片的真实请求地址，但是由于`src`属性上并没有值，所以图片并不会被加载。我们的`js`可以监听`scroll`事件，在计算到图片进入可视区域之后。就拿到`img`标签上的`data-url`属性的值，把它设置到`src`属性上，这就是懒加载的一个原理。
