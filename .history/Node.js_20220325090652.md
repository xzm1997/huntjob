...
### `V8` 执行过程
- `模块`
  - parse：负责将JavaScript源代码转换成抽象语法树（AST）
  - Ignition： interpreter，解释器，将AST转换成字节码（Bytecode），解析执行字节码，同时也收集TurboFan优化编译所需要的信息
  - TurboFan：compiler，JIT编译器，利用Ignitio所收集的类型信息，将Bytecode转换为优化的机器码
