...
- **网络请求**
  ...
  3. **DNS解析**：
    - 由于输入的是域名，而数据包是通过IP地址传给对方的。因此我们需要得到域名对应的IP地址。这个过程需要依赖一个服务系统，这个系统将域名和 IP 一一映射，这个系统就叫做DNS（域名系统）。得到具体 IP 的过程就是DNS 解析。
  ...
  5. **发送 HTTP 请求**
    ...
    - 请求头：
    - 请求体：请求体只有在POST 方法下存在，常见的场景是表单提交。
- **网络响应**
  1. 返回网络响应：跟请求部分类似，网络响应具有三个部分:响应行、响应头和响应体。
