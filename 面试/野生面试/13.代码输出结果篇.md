...
### 1. 代码输出结果 :face_with_head_bandage:
...
### 2. 代码输出结果:face_with_head_bandage:
...
### 3. 代码输出结果:face_with_head_bandage:
...
### 4. 代码输出结果:face_with_head_bandage:
...
### 5. 代码输出结果:face_with_head_bandage:
...
### 6. 代码输出结果:face_with_head_bandage:
...
### 7. 代码输出结果:face_with_head_bandage:
...
### 9. 代码输出结果:face_with_head_bandage:
...
### 10. 代码输出结果:face_with_head_bandage:
...
### 11. 代码输出结果:face_with_head_bandage:
...
### 12. 代码输出结果:face_with_head_bandage:
...
### 13. 代码输出结果:face_with_head_bandage:
...
### 14. 代码输出结果:face_with_head_bandage:
...
### 15. 代码输出结果 :face_with_head_bandage: 
...
### 16. 代码输出结果:face_with_head_bandage:
...
### 17. 代码输出结果:face_with_head_bandage:
...
### 18. 代码输出结果:face_with_head_bandage:
...
### 20. 代码输出结果:face_with_head_bandage:
...
这里首先定义了两个变量`f`和`g`，我们知道变量是可以重新赋值的。后面是一个匿名自执行函数，在 `if` 条件中调用了函数 `g()`，由于在匿名函数中，又重新定义了函数g，就覆盖了外部定义的变量`g`，所以，这里调用的是内部函数 `g` 方法，返回为 `true`。第一个条件通过，进入第二个条件。
...
4. `Foo.a()` ; 根据第2步可知 Foo 函数内部的属性方法已初始化，覆盖了同名的静态方法，所以输出：1    :red_circle:
